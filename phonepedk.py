...
cursor.execute("select * from aggregated_transc;")
mydb.commit()
...
#Map_transaction
cursor.execute("select * from map_transc")
mydb.commit()
...
#Top_transaction
cursor.execute("select * from top_transc")
mydb.commit()
...
st.title("A :violet[_PhonePe_] DATA :rainbow[VISUALIZATION] AND :green[EXPLORATION]")


selected = option_menu(
            menu_title=None,  
            options=["HOME","EXPLORE DATA", "TOP CHARTS"],  
            # icons=["house", "chart", "Yelp"],  
            menu_icon="cast",  
            default_index=0,  
...
if selected == "HOME":
    ...
    with col1:
            st.text("PhonePe is a mobile payment platform using which you\n"
                    "can transfer money using UPI, recharge phone numbers,\n"
                    "pay utility bills, etc. PhonePe works on the Unified\n"
                    "Payment Interface (UPI) system and all you need is\n"
    ...
    with col2:
        st.video("C:\\Users\\DineshMohan\\Desktop\\python\\phonepedk\\Phone Pe Ad(720P_HD).mp4")        
